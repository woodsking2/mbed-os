...
def prepare_cli_programmer_ini(cfg=None, prod_id=None, device_id=None, port=None, log=None,
    ...
    def get_gdb_server_path(jlink_path, prod_id, device_id, port, swoport, telnetport, log):
        ...
        substitutes = {
            ...
            'prod_id': 'Cortex-M33' if prod_id == ProductId.DA1469x_00 else 'Cortex-M0',
            'device_id': device_id,
            'port': str(port),
            'swoport': str(swoport),
...
class ScriptArgumentsParser(ArgumentParser):
    ...
    def add_revision_args(self):
        self.add_argument('--prod_id', '-p', type=str, metavar="PROD_ID",
                          help='device product id {}'.format([p.value for p in ProductId]),
    ...
    def parse_args(self, args=None, namespace=None):
        ...
        # Return prod_id as ProductId object instead of value string
        if getattr(args, 'prod_id', None):
            args.prod_id = ProductId(args.prod_id)
