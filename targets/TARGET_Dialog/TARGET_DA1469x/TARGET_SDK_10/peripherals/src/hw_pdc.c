...
void hw_pdc_write_entry(uint32_t idx, uint32_t value)
{
    ...
    if ((value & HW_PDC_LUT_ENTRY_FIELD_MASK(PDC_MASTER)) == 0 && ((hw_pdc_get_pending() & (1 << idx)) != 0))
    {
...
uint32_t hw_find_pdc_entry(HW_PDC_TRIG_SELECT select, HW_PDC_PERIPH_TRIG_ID id, HW_PDC_MASTER master, HW_PDC_LUT_ENTRY_EN en)
{
    uint32_t field = HW_PDC_LUT_ENTRY_VAL(select, id, master, en);
    uint32_t mask = HW_PDC_LUT_ENTRY_VAL(select, 0xf, 0x3, en);
    for (int i = 0; i < HW_PDC_LUT_SIZE; i++)
    {
