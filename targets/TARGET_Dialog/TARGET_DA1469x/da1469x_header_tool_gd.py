...
import sys
sys.path.append(os.path.join("D:/", "mcuboot", "scripts",
                "imgtool"))
import keys as keys
...
class da1469x_fw_image(object):
    ...
    def __init__(self, path, encrypt_file, sig_file, sig_slot, decrypt_slot,
                 revoke, version):
        header = product_header(self.PROD_ID, 0x2000, 0x2000, 0xa82000eb, 0x66,
                                self.FLASH_SECT_ID, 4, b'\x01\x00\x02\07')
        self.img = struct.pack('<2sIIII2sH4s', *header)
