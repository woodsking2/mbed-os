...
ram_target = [True for s in seen_sections if "stack" in s and section_data[s]["address"] > 0x0 and
              int(section_data[s]["address"], 16) < 0x80000]
...
col_id_of_retention_sections = [seen_sections.index(s) + 1 for s in seen_sections \
                                                           if "RETENTION" in s or ".retention" in s]
