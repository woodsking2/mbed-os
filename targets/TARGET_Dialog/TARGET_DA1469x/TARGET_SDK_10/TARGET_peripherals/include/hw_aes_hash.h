 ...
 *
 * Check if User Data Encryption Key haven't been revoked for corresponding index
 *
 * \param [in] idx index
 *
 ...
 *
 * Returns memory address of User Data Encryption Key for given key index
 *
 ...
 * key. Otherwise it should contain all the expanded keys. If aes_keys is taken from OTP then
 * alternative version of function is used, which may use DMA channel or simple OTP read in
 * dependence of dg_configAES_USE_SECURE_DMA_CHANNEL flag.
 *
 * \note
 * After an AES operation with a certain key, further operations using the same key do not
 * need storing it to the AES/HASH memory again. If dg_configAES_USE_SECURE_DMA_CHANNEL is set then
 * DMA channel 7 will be used to transfer key. In this case key expansion must be performed by
 * CryptoEngine to avoid storing key in RAM.
 *
 * \param[in] key_size The size of the key used in the encryption/decryption
